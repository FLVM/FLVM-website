...
export const load: PageLoad = async ({ fetch, data }) => {
  ...
  const calendarApiResponse = await fetch(url + new URLSearchParams(params).toString())
    .then(result => {
      // console.log("RESULT", result)
      return result.json()
    })
  const eventsCalendar: Array<CalendarEventType> = calendarApiResponse.items.map((i: CalendarEventType) => ({
    id: i.id,
    status: i.status,
    htmlLink: i.htmlLink,
    created: i.created,
...
function getPrettyDate(date: string, locale?: Intl.LocalesArgument, options?: Intl.DateTimeFormatOptions) {
...
function getTitle(string: string) {
  ...
  let title = string.replace(/cuisine|pâtisserie|confiserie|enfants|:/ig, "").trim()
  return title.charAt(0).toUpperCase() + title.slice(1)
}

function getTags(string: string) {
  const tags: Array<string> = []
  const words: Array<string> = string.split(/[\s:]+/).filter(w => w.trim() !== ":").map(w => w.toLocaleLowerCase())
  wordsList.forEach(w => {
