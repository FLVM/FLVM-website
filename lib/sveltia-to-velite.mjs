...
function fieldToZod(field) {
  ...
  switch (field.widget) {
    case "boolean" : result += `${name}: s.boolean()`; break;
    case "number" : result += `${name}: s.number()`; break;
    case "datetime": result += `${name}: s.date()`; break;
    // @todo : voir pourquoi s.markdown() génère : 
    // " info The content is empty  image.legend "
    case "markdown": result += `${name}: s.markdown()`; break;
...
export async function generateCollections(config = {}) {
  ...
  const collections = raw.collections.concat(raw.singletons.map(s => ({...s, singleton: true})))
  ...
  for (const c of collections.filter(c => !c.divider)) {
    ...
    const schema = Object.values(c.fields)
    ...
    result.push(`export const ${c.name} = { name: ${JSON.stringify(id)}, pattern: ${JSON.stringify(pattern)}, single: ${single}, schema: ${id}Schema}`)
    result.push("")
