...
function objectPropsToZod(props) {
  const entries = Object.entries(props).map(([k,v])=>{
    const expr = (v.type === 'object') ? objectPropsToZod(v.properties || {}) : (v.type === 'array' ? (v.items && v.items.type === 'object' ? `z.array(z.object(${objectPropsToZod(v.items.properties||{})}))` : (typeMap[v.items?.type]||'z.any()')) : (typeMap[v.type]||'z.any()'));
...
async function main() {
  ...
  for (const s of schemas) {
    ...
    parts.push(`export const ${id} = { id: ${JSON.stringify(s.id)}, path: ${JSON.stringify(s.path || `content/${s.id}`)}, schema: ${id}Schema };`);
    parts.push('');
  }
